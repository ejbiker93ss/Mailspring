...
class NativeNotifications {
  ...
  /**
   * Get notification icon. Only works on linux, otherwise the SummerMail default icon wil be read
   * from resources.
   *
   * Reading the icon name from the desktop file of SummerMail. If the icon is a name, reads the
  ...
  private buildWindowsToastXml(options: {
    ...
    const actionsContent = (options.actions || [])
      .map((action, i) => {
        const actionUrl = `summermail://notification-action?${baseParams}&actionIndex=${i}`;
    ...
    const clickUrl = `summermail://notification-click?${baseParams}`;
