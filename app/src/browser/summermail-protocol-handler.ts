...
// Handles requests with 'summermail' protocol.
//
// It's created by {Application} upon instantiation and is used to create a
// custom resource loader for 'summermail://' URLs.
//
...
//
export default class SummerMailProtocolHandler {
  loadPaths: string[] = [];
  ...
  // Creates the 'SummerMail' custom protocol handler.
  registerProtocol() {
    for (const scheme of ['summermail', 'mailspring']) {
