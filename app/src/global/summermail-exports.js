...
const lazyLoadWithGetter = (prop, getter) => {
  ...
  if (exports[key]) {
    throw new Error(`Fatal error: Duplicate entry in summermail-exports: ${key}`);
  }
...
// API Endpoints
lazyLoad(`SummerMailAPIRequest`, 'flux/summermail-api-request');
...
lazyLoadWithGetter(
  `SummerMailTestUtils`,
  () => require('../../spec/summermail-test-utils').default
);
