...
import { Rx } from 'summermail-exports';
...
// only exists once `summermail-observables` has run and patched it onto Rx.
// That normally happens because app-env.ts requires 'summermail-observables'
// before 'summermail-exports', but nothing enforces that ordering when this
...
class TaskQueue extends SummerMailStore {
  _queue: Task[] = [];
  _completed: Task[] = [];
