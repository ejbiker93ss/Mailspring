...
describe('ApplicationMenu', function () {
  describe('translateTemplate', function () {
    it('leaves contextual command accelerators to the renderer on Windows and Linux', function () {
      ...
      expect(template[0].registerAccelerator).toBe(
        process.platform === 'darwin' ? undefined : false
      );
      expect(template[1].accelerator).toBe('Ctrl+U');
      expect(template[1].registerAccelerator).toBe(
        process.platform === 'darwin' ? undefined : false
      );
