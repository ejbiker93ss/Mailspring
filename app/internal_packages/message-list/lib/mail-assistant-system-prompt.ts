...
export function buildMailAssistantInstructions(options: {
  ...
  return (
    [
      ...
      'Read tools may be used automatically. Email drafts, calendar events, message moves, deleting messages, and marking messages as read are proposals only: the user must review and confirm them in Mailspring. Deleting means moving the selected threads to Trash. Never send mail or mutate mailbox state without that confirmation.',
