...
Build a changelog entry for a new SummerMail release by analyzing git history.
...
7. **Add a release entry** to the top of the `<releases>` list in `app/build/resources/linux/summermail.metainfo.xml.in`, without reformatting the rest of the file:
   ```xml
